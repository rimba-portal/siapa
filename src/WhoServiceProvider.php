...
use Rimba\Who\Services\{AuthOrchestratorService, FaceAuthService, LocalAuthService, PanelAccessService, SecurityContextService, StaffResolverService};
    ...
    protected function registerPackage(): void
    {
        ...
        $this->app->singleton(AuthOrchestratorContract::class, function ($app): AuthOrchestratorContract {
                ...
                foreach (config('bites_auth.authentication.providers', ['local']) as $name) {
                    if ($name === 'local') { $providers[] = $app->make(LocalAuthService::class); } elseif ($app->bound("bites_auth.auth-provider.$name")) { $providers[] = $app->make("bites_auth.auth-provider.$name"); } }
